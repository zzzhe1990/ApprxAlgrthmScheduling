...
int DPFunction2(vector<int>& Ntemp)
{
    ...
    cout << "all table elemets size: " << AllTableElemets.size() << ", ATE elm size: " << AllTableElemets.elm.size() << 
    
    
	cudaMalloc((void**)&dev_ATE_Csubsets, AllTableElemets.size() * Cwhole.size() * pow(k,2) * sizeof(int));
	cudaMalloc((void**)&dev_ATE_NSsubsets, AllTableElemets.size() * Cwhole.size() * pow(k,2) * sizeof(int));
	cudaMalloc((void**)&dev_ATE_elm, AllTableElemets.size() * pow(k,2) * sizeof(int));
	cudaMalloc((void**)&dev_ATE_myMinNSVector, AllTableElemets.size() * pow(k,2) * sizeof(int));
	cudaMalloc((void**)&dev_ATE_myOPT, AllTableElemets.size() * sizeof(int));
	cudaMalloc((void**)&dev_ATE_myOptimalindex, AllTableElemets.size() * sizeof(int));
	cudaMalloc((void**)&dev_ATE_optVector, AllTableElemets.size() * pow(k,2), sizeof(int));
	cudaMalloc((void**)&dev_counterVec, (LongJobs.size() + 1) * sizeof(int));
	cudaMalloc((void**)&dev_zeroVec, (pow(k,2)) * sizeof(int));
	cudaMalloc((void**)&dev_roundVec, (pow(k,2)) * sizeof(int));
	
	//size for arrays
	//int dev_ATE_elm_size = AllTableElemets.size() * pow(k,2);
	int dev_AllTableElemets_size = AllTableElemets.size();
	int ATE_NSsubsets_size[AllTableElemets.size()];
	int ATE_optVector_size[AllTableElemets.size()];
	...
	cudaMalloc((void**)&dev_ATE_NSsubsets_size, AllTableElemets.size() * sizeof(int));
    cudaMalloc((void**)&dev_ATE_optVector_size, AllTableElemets.size() * sizeof(int));
    int powK = pow(k,2);
   
	//cuda memory copy
	cudaMemcpy(dev_ATE_elm, AllTableElemets.elm, AllTableElemets.size() * pow(k,2) * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_zeroVec, zeroVec, pow(k,2)*sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_roundVec, roundVec, pow(k,2)*sizeof(int), cudaMempcyHostToDevice);
	cudaMemcpy(dev_ATE_NSsubsets_size, ATE_NSsubsets_size, AllTableElemets.size() * sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(dev_ATE_optVector_size, ATE_optVector_size, AllTableElemets.size() * sizeof(int), cudaMemcpyHostToDevice);
		
    while (i < maxSumValue+1)		//number of levels = number of jobs + 1
    {
//#pragma omp parallel shared (AllTableElemets,indexomp,i,counterVec)  num_threads(nthreads0)
//#pragma omp for schedule(static,1)
		int tSize = 64;
		int bSize = 1;
		if (tSize > counterVec[i])
			bSize = (tSize + counterVec[i] - 1) / counterVec[i];

		FindOPT<<<bSize, tSize>>>(dev_ATE_elm, dev_counterVec, indexomp, dev_roundVec, T, k, powK, dev_AllTableElemets_size, dev_ATE_Csubsets, dev_ATE_NSsubsets, dev_ATE_NSsubsets_size, 
						Cwhole.size(), dev_zeroVec, dev_ATE_optVector, dev_ATE_optVector_size, dev_ATE_myOPT, dev_ATE_myOptimalindex, dev_ATE_myMinNSVector);
                
        cudaMemcpy(counterVec, dev_counterVec, powK * sizeof(int), cudaMemcpyDeviceToHost);
...
//GPU code to update AllTableElement
	for(int i=0; i < AllTableElemets.size(); i++){
		cudaMemcpy(AllTableElemets[i].NSsubsets, &dev_ATE_NSsubsets[i * Cwhole.size() * powK], Cwhole.size() * powK, cudaMemcpyDeviceToHost);
		cudaMempcy(AllTableElemets[i].optVector, &dev_ATE_optVector[i * powK], powK, cudaMemcpyDeviceToHost);
		//Csubsets[Cwhole.size()][powK]
		cudaMemcpy(AllTableElemets[i].Csubsets, &dev_ATE_Csubsets[i * Cwhole.size() * powK], Cwhole.size() * powK, cudaMemcpyDeviceToHost);
	}
