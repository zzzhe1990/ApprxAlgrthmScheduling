...
__global__ void FindOPT(int *dev_ATE_elm, int *dev_counterVec, int indexomp, int *dev_roundVec,
						...
						const int cpuId, const int configOffset, const int blockLvl, 
...
/*		
		int blockSize = 32;
		int gridSize = (dev_ATE_NSsubsets_size[j] + 31) / 32;
...
*/		
		...
		{
//			if (optOffset + pp >= AllTableElemets_size*optVectorSize)
//				printf("cpuId: %d, job ID: %d, optOffset: %d, pp: %d, optVector_size: %d, dev_ATE_optVector_size[%d]: %d, dev_ATE_NSsubsets_size[%d]: %d, access of dev_ATE_optVector is out of bound.\n", 
//				cpuId, j, optOffset, pp, AllTableElemets_size*optVectorSize, j, dev_ATE_optVector_size[j], j, dev_ATE_NSsubsets_size[j]);
							...
							const int blockIDInLvl)
{
	...
	//int *bIfSame = &dev_ifSame[configOffset];
...
*/
	...
	//gpuErrchk(cudaMemsetAsync(dev_ifSame, 0, AllTableElemets.size()*sizeof(int), streams[thread*batchSize]));
